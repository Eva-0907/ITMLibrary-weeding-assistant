...
def unicat_url(rec):
    isbn = get_isbn(rec)
    if isbn:
        return f"https://www.unicat.be/uniCat?func=find-b&find_code=ISB&request={isbn}"
    title  = " ".join(gf(rec, "T1", "TI").split()[:4])
    author = " ".join(gf(rec, "A1", "A2").split()[:4])
    q = " ".join(filter(None, [title, author]))
    import urllib.parse
    return f"https://www.unicat.be/uniCat?func=find-b&find_code=WRD&request={urllib.parse.quote(q)}"
...
def main():
    ...
    parser.add_argument("--out",             default="weeding_report1.xlsx", help="Output XLSX filename")
    ...
    if ris_text is None:
        ...
        ris_text = Path(args.ris).read_text(encoding="latin-1")
        print("  Encoding detected: latin-1 (fallback)")
