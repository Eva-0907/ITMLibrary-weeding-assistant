...
Changes in v9 vs v8:
  ...
  - Fixed a follow-on bug from the above: the translation-duplicate decision sets
    keep_override = False, but several rules running afterward (class scarcity
    protection, older-edition, and the date-based weed rules) only checked
    "if not keep_override" — and would themselves set keep_override back to True
    (e.g. class scarcity, pre-1950) without knowing that False had just been a
    deliberate final decision rather than "nothing decided yet." This silently
    reversed the translation fix for any record that also happened to trigger
    scarcity protection or the pre-1950 rule. A translation_final flag now guards
    all of these downstream rules so the translation-duplicate WEED decision is
    truly final once made.
...
def apply_rules(rec, all_records, borrowed_bibs, isbn_counts, older_edition=False, translation_duplicate=False, barnard_counts=None):
    ...
    # This is treated as a FINAL decision — later rules (older edition, scarcity,
    # WHO/FAO) must not be able to flip it back to a keep just because they see
    # keep_override = False and assume nothing has been decided yet.
    ...
    if older_edition and not keep_override and not translation_final:
    ...
    if not keep_override and not translation_final and barnard_counts is not None:
    ...
    # ── Weed rules (only if not already kept, and not already finalized by translation) ──
    if not keep_override and not translation_final:
        # Duplicate
