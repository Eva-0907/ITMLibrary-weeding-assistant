...
# ── Rule engine ──────────────────────────────────────────────
def apply_rules(rec, all_records, borrowed_bibs, isbn_counts, older_edition=False, translation_duplicate=False):
    year          = get_year(rec)
    isbn          = get_isbn(rec)
...
def main():
    ...
    parser.add_argument("--out",             default="weeding_report3.xlsx", help="Output XLSX filename")
    ...
    for i, rec in enumerate(records):
        result = apply_rules(rec, records, borrowed, isbn_counts, older_edition=(i in older_edition_indices), translation_duplicate=(i in translation_weed_indices))
        rec_val = result["recommendation"]
