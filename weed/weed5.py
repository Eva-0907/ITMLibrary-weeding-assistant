...
OUTBREAK_TIMELINE = [
    {"event": "First smallpox vaccination (Jenner)",         "keywords": ["smallpox","jenner","variol","vaccinia","cowpox"]},
    {"event": "Broad Street cholera outbreak (John Snow)",   "keywords": ["cholera","john snow","broad street","vibrio"]},
    ...
    {"event": "Malaria eradication campaigns",               "keywords": ["malaria","ddt","anopheles","plasmodium"]},
    {"event": "Global smallpox eradication programme",       "keywords": ["smallpox","eradication","vaccinia"]},
...
def is_historical_title(title):
    ...
    for ht in HISTORICAL_TITLES:
        if ht in t:
            # Require the match to cover at least 60% of the actual title words
            # This prevents short terms like "clinical parasitology" matching
            # "A colour atlas of clinical parasitology"
            ht_words = len(ht.split())
            t_words  = len(t.split())
            if t_words == 0:
                continue
            coverage = ht_words / t_words
            if coverage >= 0.6:
                return True
        # Also allow: historical title starts with first 20 chars of record title
        if len(t) >= 20 and ht.startswith(t[:20]):
            return True
    return False
...
def apply_rules(rec, all_records, borrowed_bibs, isbn_counts, older_edition=False, translation_duplicate=False, barnard_counts=None):
    ...
    # ── Tropical / Africa / developing countries keyword rule ──
    TROPICAL_KWS = {
        # Tropical medicine & disease-specific
        'tropical medicine', 'tropical diseases', 'tropical',
        'malaria', 'parasitic diseases', 'protozoal diseases', 'helminthic diseases',
        'leprosy', 'schistosomiasis', 'trypanosomiasis', 'leishmaniasis', 'filariasis',
        'onchocerciasis', 'sleeping sickness', 'vector-borne diseases',
        # Africa — continental and regional
        'africa', 'africa-central', 'africa-east', 'africa-west', 'africa-south', 'africa-north',
        # African countries
        'congo-kinshasa', 'congo-brazzaville', 'rwanda', 'burundi', 'uganda', 'kenya',
        'tanzania', 'ethiopia', 'ghana', 'nigeria', 'senegal', 'mali', 'niger', 'cameroon',
        'malawi', 'zambia', 'zimbabwe', 'mozambique', 'angola', 'sudan', 'somalia',
        'sierra leone', 'liberia', 'guinea', 'gambia', 'togo', 'benin', 'chad', 'tchad',
        'madagascar', 'egypt', 'botswana', 'namibia', 'lesotho', 'swaziland', 'eritrea',
        'djibouti', 'comoros', 'mauritania', 'burkina faso', 'guinea-bissau',
    }
    GENERIC_KWS = {
        'general', 'manual', 'techniques', 'training', 'methods', 'textbook',
        'introduction', 'handbook', 'guide', 'reference',
    }
    kw_list = rec.get('KW', [])
    kw_list = [kw_list] if isinstance(kw_list, str) else kw_list
    kws_lower = {k.lower().strip() for k in kw_list}
    has_tropical_kw = bool(kws_lower & TROPICAL_KWS)
    has_generic_kw  = bool(kws_lower & GENERIC_KWS)

    if has_tropical_kw and not has_generic_kw and not keep_override:
        flag("Tropical keyword", "Has tropical/Africa/developing-countries keyword — keep as representative of tropical collection", "keep")
        keep_override = True
        had_keep_reason = True
    ...
    if translation_duplicate and not keep_override:
        flag("Translation", "Non-English version — English edition exists in collection — weed", "weed")
        hard_rule = "WEED"
    elif translation_duplicate and keep_override:
        # Even if it would be kept, downgrade to REVIEW so human can verify
        keep_override = False
        hard_rule = "REVIEW"
        flag("Translation", "Non-English version — English edition exists — verify if physical copy needed", "review")
    ...
    if is_who_fao and (keep_override or had_keep_reason):
        keep_override = False
        hard_rule = "REVIEW"
...
def main():
    ...
    parser.add_argument("--out",             default="weeding_report5.xlsx", help="Output XLSX filename")
